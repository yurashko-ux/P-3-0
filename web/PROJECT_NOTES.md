...
## На чому зупинилися
- ✅ **ВИРІШЕНО**: Автоматичне переміщення карток під час ManyChat-автоматизації працює коректно. Проблема була у передачі `pipeline_status_id` або неправильному payload у автоматизації — виправлено шляхом уніфікації побудови запиту в `routeManychatMessage` з ручним тестом.
- ✅ **ВИРІШЕНО (2025-01-15)**: EXP (експірація) — виправлено фільтрацію карток. Проблема: переміщувалися всі картки з базової воронки, а не тільки з базового статусу. Виправлення: оновлено формат запиту до KeyCRM API в `web/lib/exp-check.ts` — використовується `filter[status_id]` замість `status_id`, додано `include[]`/`with[]` параметри, покращено ручну фільтрацію за статусом.
...
### Altegio інтеграція
- **Всі дані отримуємо через Altegio API**. Вебхуки використовуємо лише як тригери (сигнал про зміну).
- При отриманні вебхука викликаємо відповідний API endpoint і зберігаємо відповідь у БД.
- **Fallback для custom_fields (Instagram):** якщо API не повертає `custom_fields` або вони порожні, беремо їх з payload вебхука.
- **Fallback при помилці API:** якщо API не відповідає або повертає помилку, використовуємо payload вебхука як резервне джерело даних.
- Відповіді вебхуків логуються в **Vercel KV (Redis-сумісне сховище)** під ключем `altegio:webhook:log` (останні 50 записів).
- Якщо Instagram не визначено (ні в API, ні в webhook), створюємо клієнта з `missing_instagram_${clientId}`.
...
- ✅ Автоматичне переміщення карток працює стабільно після виправлення.
- Продовжити роботу над модулем аналітики Alteg.io (див. розділ вище).
- При потребі — зберігати логи переміщень для аналітики та діагностики.
