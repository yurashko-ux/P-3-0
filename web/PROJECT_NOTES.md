...
### Altegio інтеграція
- **НЕ використовуємо Altegio API для отримання даних про клієнтів**. 
- Всі дані отримуємо виключно з webhook-ів, які зберігаються в **Vercel KV (Redis-сумісне сховище)** під ключем `altegio:webhook:log` (останні 50 записів).
- Instagram username витягується виключно з даних, що приходять в webhook.
- Якщо Instagram не прийшов в webhook, створюємо клієнта з `missing_instagram_${clientId}`.
- Причини: Altegio API не повертає потрібні поля (custom_fields) у правильному форматі або взагалі не працює.
- Примітка: Vercel KV - це Redis-сумісна база даних, але не Postgres. Webhook-и зберігаються саме в Redis (Vercel KV), а не в Postgres.
