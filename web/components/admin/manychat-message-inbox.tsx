        ...
        <div className="rounded-xl border border-dashed border-amber-200 bg-amber-50/60 p-4">
          ...
          {apiDiag ? (
            <p className="mt-2 text-sm text-amber-700">
              {apiDiag.ok
                ? `✅ Дані отримано (${apiDiag.note === "empty" ? "повідомлення відсутні" : apiDiag.note === "merged" ? "об'єднано з KV" : "є нові повідомлення"})`
              ...
              {apiDiag.ok && typeof apiDiag.merged === 'number' ? (
                <span className="block text-xs text-amber-600/70">
                  Поточний список: {apiDiag.merged} запис(ів)
                </span>
              ) : null}
            </p>
          ) : (
