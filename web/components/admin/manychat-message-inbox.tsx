...
const toTargetSummaryFromDetails = (input: unknown): TargetSummary => {
  if (!input || typeof input !== "object") {
    return { pipelineId: null, statusId: null, pipelineName: null, statusName: null };
  }
  ...
  if (!automationResult && !automationAnalysis) {
    ...
    if (!campaignSource) {
      ...
      if (errorSource) {
        ...
        if (errorSource.error === "keycrm_move_failed") {
        ...
        } else if (detailsCampaign) {
          const campaignName =
            coerceString(detailsCampaign.name) ??
            coerceString(detailsCampaign.title) ??
            coerceString(detailsCampaign.id) ??
            "(без назви)";
          const baseSummary = toTargetSummaryFromDetails(detailsCampaign.base ?? null);
          const targetSummary = toTargetSummaryFromDetails(
            detailsCampaign.target ??
              (detailsCampaign.t1 as unknown) ??
              (detailsCampaign.t2 as unknown) ??
              null,
          );
          ...
          const baseLabel =
            ...
            coerceString(detailsCampaign.base_pipeline_name) ??
            coerceString(detailsCampaign.base_pipeline_id) ??
            "—";
          const baseStatusLabel =
            baseSummary.statusName ??
            baseSummary.statusId ??
            coerceString(detailsCampaign.base_status_name) ??
            coerceString(detailsCampaign.base_status_id) ??
            "—";
          campaignNotes.push(
          ...
          const targetLabel =
            ...
            coerceString(detailsCampaign.target_pipeline_name) ??
            coerceString(detailsCampaign.target_pipeline_id) ??
            coerceString(detailsCampaign.v1_to_pipeline_name) ??
            coerceString(detailsCampaign.v1_to_pipeline_id) ??
            coerceString(detailsCampaign.v2_to_pipeline_name) ??
            coerceString(detailsCampaign.v2_to_pipeline_id) ??
            "—";
          const targetStatusLabel =
            targetSummary.statusName ??
            targetSummary.statusId ??
            coerceString(detailsCampaign.target_status_name) ??
            coerceString(detailsCampaign.target_status_id) ??
            coerceString(detailsCampaign.v1_to_status_name) ??
            coerceString(detailsCampaign.v1_to_status_id) ??
            coerceString(detailsCampaign.v2_to_status_name) ??
            coerceString(detailsCampaign.v2_to_status_id) ??
            "—";
          campaignNotes.push(
      ...
      } else if (automationResult.ok) {
        const moveDetails: ReactNode[] = [];
        let moveStatus: TimelineStatus = automationResult.move.ok ? "success" : "warning";

        if (automationResult.move.attempted) {
          moveDetails.push(
            <span key="attempt">Переміщення: {automationResult.move.ok ? "✅ успішно" : "⚠️ не підтверджено"}</span>,
          );
          if (automationResult.move.sent) {
            moveDetails.push(
              <span key="payload" className="text-xs text-slate-600/80">
                Надіслано: {JSON.stringify(automationResult.move.sent)}
              </span>,
            );
          }
          if (automationResult.move.attempts && automationResult.move.attempts.length) {
            const lastAttempt = automationResult.move.attempts[automationResult.move.attempts.length - 1];
            moveDetails.push(
          ...
          if (automationResult.move.status) {
            moveDetails.push(
              <span key="status" className="text-xs text-slate-600/70">Код відповіді KeyCRM: {automationResult.move.status}</span>,
            );
          }
