...
export default function PipelinesRefreshButton() {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const statusCount = pipelines.reduce((total, pipeline) => {
        if (!pipeline || !Array.isArray(pipeline.statuses)) return total;
        return total + pipeline.statuses.length;
      }, 0);
      setState('success');
      setMessage(
        pipelineCount > 0
          ? `Оновлено ${pipelineCount} воронок та ${statusCount} статусів із KeyCRM.`
