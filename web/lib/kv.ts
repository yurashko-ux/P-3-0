...
async function kvGetRaw(key: string) {
  ...
  const pathCandidates = [`get/${encoded}`, `kv/${encoded}`, `v0/kv/${encoded}`];

  for (const path of pathCandidates) {
    const res = await rest(path, {}, true, true).catch(() => null);
    if (!res) continue;
    if (res.status === 404) {
      continue;
    }
...
async function kvSetRaw(key: string, value: string) {
  ...
  const encoded = encodeURIComponent(key);
  const pathCandidates = [`set/${encoded}`, `kv/${encoded}`, `v0/kv/${encoded}`];
  ...
  for (const path of pathCandidates) {
    try {
      ...
      if (status !== 405 && status !== 400 && status !== 501 && status !== 404) {
        throw error;
      }
      ...
      try {
        ...
        const errStatus = typeof (err as any)?.status === 'number' ? (err as any).status : null;
        if (errStatus !== 405 && errStatus !== 400 && errStatus !== 501 && errStatus !== 404) {
          throw err;
        }
