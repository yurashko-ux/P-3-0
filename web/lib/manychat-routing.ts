...
type MoveResult = {
  ...
  trace?: KeycrmMoveTraceEntry[] | null;
  stack?: string | null;
};
...
const readTarget = (
  ...
  const pipelineStatusId =
    ...
    (fallbackPipelineStatus
      ? toId(campaign?.[fallbackPipelineStatus])
      : null) ??
    ...
    (fallbackPipelineStatus
      ? toId(campaign?.[`${fallbackPipelineStatus}Id`])
      : null);
  ...
  if (fallbackPipelineStatus) {
    pushAlias(campaign?.[fallbackPipelineStatus]);
    pushAlias(campaign?.[`${fallbackPipelineStatus}Id`]);
    pushAlias(campaign?.[`${fallbackPipelineStatus}_id`]);
  }
  ...
  let resolvedPipelineStatusId = pipelineStatusId;
  if (!resolvedPipelineStatusId && statusId) {
    resolvedPipelineStatusId = statusId;
  }

  if (resolvedPipelineStatusId && !statusAliases.includes(resolvedPipelineStatusId)) {
    statusAliases.push(resolvedPipelineStatusId);
  }
  ...
  return {
    pipelineId,
    statusId,
    pipelineStatusId: resolvedPipelineStatusId,
    pipelineName,
    statusName,
    statusAliases,
  };
...
export async function routeManychatMessage({
  ...
  let target = readTarget(
    ...
    chosen.route === 'v1'
      ? 'v1_to_pipeline_status_id'
      : 'v2_to_pipeline_status_id',
  );
  ...
  if (base?.pipelineStatusId && !base.statusId) {
    base = { ...base, statusId: base.pipelineStatusId };
  }

  if (target?.pipelineStatusId && !target.statusId) {
    target = { ...target, statusId: target.pipelineStatusId };
  }
  ...
  const plannedPipelineId =
    normalizeId(target.pipelineId) ?? normalizeId(selected.summary?.pipelineId) ?? normalizeId(base.pipelineId);
  const plannedPipelineStatusId = normalizeId(target.pipelineStatusId);
  const plannedStatusId = normalizeId(target.statusId);
  const plannedAliasCandidates = (
    [target.pipelineStatusId, target.statusId, ...(target.statusAliases ?? [])] as Array<unknown>
  )
    .map((value) => normalizeId(value))
    .filter((value): value is string => Boolean(value));
  const plannedStatusAliases = Array.from(
    new Set(
      plannedAliasCandidates.filter(
        (alias) => alias !== plannedPipelineStatusId && alias !== plannedStatusId,
      ),
    ),
  );

  const plannedRequestBody: Record<string, unknown> = { card_id: cardIdStr };

  if (plannedPipelineId) {
    plannedRequestBody.to_pipeline_id = plannedPipelineId;
    plannedRequestBody.pipeline_id = plannedPipelineId;
  }

  if (plannedStatusId) {
    plannedRequestBody.to_status_id = plannedStatusId;
    plannedRequestBody.status_id = plannedStatusId;
  }

  if (plannedPipelineStatusId) {
    plannedRequestBody.pipeline_status_id = plannedPipelineStatusId;
  }

  if (plannedStatusAliases.length) {
    plannedRequestBody.status_aliases = plannedStatusAliases;
  }

  const plannedRequestPath = `/pipelines/cards/${encodeURIComponent(cardIdStr)}`;
  const plannedRequestMethod = 'PUT';
  ...
  if (alreadyInTarget) {
    return success({
      ...
      move: {
        ...
        ok: true,
        requestUrl: plannedRequestPath,
        requestMethod: plannedRequestMethod,
        sent: plannedRequestBody,
        requestHeaders: null,
        requestBodyRaw: null,
        responseRaw: null,
        trace: [],
  ...
  const primaryStatusId =
    directTargetStatusId ?? targetStatusCandidates.find((candidate) => candidate.length > 0) ?? null;

  const statusIdForMove = primaryPipelineStatusId ?? primaryStatusId ?? null;
  ...
  for (const alias of aliasCandidates) {
    if (!alias) continue;
    if ((statusIdForMove && alias === statusIdForMove) || (primaryPipelineStatusId && alias === primaryPipelineStatusId)) {
      continue;
    }
    targetAliasSet.add(alias);
  }

  const aliasArray = Array.from(targetAliasSet);
  if (aliasArray.length) {
    plannedRequestBody.status_aliases = aliasArray;
  }
    ...
    return success({
      ...
      move: {
        ...
        details: {
          ...
          requestUrl: moveResult.requestUrl ?? plannedRequestPath,
          requestMethod: moveResult.requestMethod ?? plannedRequestMethod,
          baseUrl: moveResult.baseUrl ?? null,
          sent: moveResult.sent ?? plannedRequestBody,
          requestHeaders: moveResult.requestHeaders ?? null,
          requestBodyRaw: moveResult.requestBodyRaw ?? null,
          responseRaw: moveResult.responseRaw ?? null,
        },
        ...
        requestBodyRaw: moveResult.requestBodyRaw ?? null,
        responseRaw: moveResult.responseRaw ?? null,
        trace: moveResult.trace ?? [],
        stack: moveResult.stack ?? null,
      },
    });
  }
