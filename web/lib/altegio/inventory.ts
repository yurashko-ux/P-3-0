...
export type SoldGoodItem = {
  ...
  categoryTitle?: string; // Категорія/підкатегорія/пошуковий текст товару з Altegio, якщо API його повертає
 ...
 * Решта (бренди, Luna, аксесуари, гребінці) — звичайний товар.
 */
...
function normalizeHairProductTitleKey(value: unknown): string {
  return normalizeHairGoodsText(value)
    .replace(/^товар\s*#\s*/u, "")
    .replace(/\s+/g, "")
    .replace(/[._\-/]+/g, "-")
    .replace(/^-+|-+$/g, "");
...
function getGoodCategoryTitle(source: any): string | undefined {
  ...
  visit(source, 0);
  visit(source?.good, 0);
  visit(source?.product, 0);
  visit(source?.category, 0);
  visit(source?.categories, 0);
...
function applyHairCategoryMatchesToGoodsMap(
  goodsMap: Map<number | string, SoldGoodItem>,
  hairProductIds: Set<number>,
  hairProductTitleKeys: Set<string> = new Set(),
): void {
  if ((hairProductIds.size === 0 && hairProductTitleKeys.size === 0) || goodsMap.size === 0) return;
  for (const item of goodsMap.values()) {
    const titleKey = normalizeHairProductTitleKey(item.title);
    if ((item.goodId && hairProductIds.has(item.goodId)) || (titleKey && hairProductTitleKeys.has(titleKey))) {
...
export type HairGoodsCategoryMatch = {
  source: "product_categories" | "product_category_detail" | "goods_search";
  id?: number;
  title: string;
...
export type HairGoodsDiagnostics = {
  ...
  selectedProductsSample: Array<{ id?: number; title: string }>;
};
...
export type GoodsSalesSummary = {
  ...
  hairGoodsDiagnostics?: HairGoodsDiagnostics;
  /**
...
function getProductIdFromCategoryProduct(product: any): number {
...
function getTitleFromCategoryProduct(product: any): string {
  const attrs = product?.attributes && typeof product.attributes === "object" ? product.attributes : {};
  return String(product?.title ?? product?.name ?? attrs.title ?? attrs.name ?? "").trim();
}

function collectProductIdsFromCategoryEntry(entry: any): Set<number> {
  ...
  const addProductsArray = (products: unknown) => {
    if (!Array.isArray(products)) return;
    for (const product of products) {
      add(getProductIdFromCategoryProduct(product));
    }
  };

  addProductsArray(entry?.products);
  addProductsArray(attrs.products);
  addProductsArray(entry?.items);
  addProductsArray(attrs.items);

  const relationshipProducts = entry?.relationships?.products?.data;
  if (Array.isArray(relationshipProducts)) {
    for (const ref of relationshipProducts) add(ref?.id);
  }
...
function normalizeProductCategory(
  ...
  const parentIdRaw =
    ...
    attrs.parent_id ??
    entry.relationships?.parent_category?.data?.id ??
    entry.relationships?.parent?.data?.id ??
    null;
  const parentId = Number(parentIdRaw);
  const title = String(attrs.title ?? attrs.name ?? entry.title ?? entry.name ?? "").trim();
...
function getIncludedProductCategoryId(product: any): number {
  ...
  const id = Number(
    product?.category_id ??
      ...
      product?.relationships?.product_category?.data?.id ??
      0,
  );
  return Number.isFinite(id) && id > 0 ? id : 0;
}
...
async function fetchHairProductIdsFromProductCategories(
...
): Promise<HairProductCategoryLookupResult> {
  ...
  try {
    ...
    const data = getJsonApiData(raw);
    const categoryRows = Array.isArray(data) ? data : data && typeof data === "object" ? [data] : [];
    ...
    for (const category of categories.values()) {
      if (isHairCategoryTitle(category.title)) hairCategoryIds.add(category.id);
    }
    ...
    while (expanded) {
      expanded = false;
      for (const category of categories.values()) {
        if (category.parentId && hairCategoryIds.has(category.parentId) && !hairCategoryIds.has(category.id)) {
...
async function fetchHairProductIdsFromProductCategoryDetails(
  locationId: string,
  categoryMatches: HairGoodsCategoryMatch[],
  soldProductIds: number[],
): Promise<HairProductCategoryLookupResult> {
  const soldIds = new Set(soldProductIds.filter((id) => Number.isFinite(id) && id > 0));
  const matchedProductIds = new Set<number>();
  ...
  for (const categoryId of ids) {
    try {
      const raw = await altegioFetch<any>(
        `/locations/${locationId}/product_categories/${categoryId}?include=products`,
        {},
        5,
        350,
        45000,
      ...
      for (const row of rows) {
        ...
        if (category) {
          ...
          for (const productId of category.productIds) categoryProductIds.add(productId);
        }
      }
      ...
      const soldProductMatches = Array.from(categoryProductIds).filter((id) => soldIds.has(id)).length;
      ...
      categories.push({
        ...
        id: categoryId,
        title: categoryTitle || fallbackTitle || `Категорія #${categoryId}`,
        parentId,
        ...
        productTitlesSample: categoryProductTitles.slice(0, 12),
      });

      console.log(
        `[altegio/inventory] ✅ Product category ${categoryId} include=products: products=${categoryProductIds.size}, sold=${soldProductMatches}`,
      );
      await new Promise((resolve) => setTimeout(resolve, 80));
    } catch (err: any) {
      console.warn(
        `[altegio/inventory] ⚠️ Не вдалося отримати product_categories/${categoryId}?include=products для волосся:`,
        err?.message || String(err),
      );
    }
  }
...
function collectGoodsSearchCategoryDiagnostics(
  ...
  const allCategories = Array.from(categories.entries())
    .map(([id, category]) => ({
      ...
      productIdsSample: Array.from(category.productIds).slice(0, 12),
      soldProductIdsSample: Array.from(category.productIds).filter((id) => soldIds.has(id)).slice(0, 12),
      productTitlesSample: category.productTitles.slice(0, 12),
    }))
    .sort((a, b) => a.title.localeCompare(b.title, "uk-UA"));
...
export async function fetchGoodsSalesSummary(params: {
  ...
  if (sales.length > 0) {
    try {
      ...
      hairGoodsDiagnostics = {
        goodsMapProductIds: goodsMapProductIds.length,
        ...
        selectedProductsSample: Array.from(goodsMap.values())
          .filter((item) => item.goodId && soldProductIds.includes(item.goodId))
          .slice(0, 20)
          .map((item) => ({ id: item.goodId, title: item.title })),
      };
      console.log(
      ...
      const hairCategoryDetailResult = await fetchHairProductIdsFromProductCategoryDetails(
        companyId,
        hairCategoryMatches,
        soldProductIds,
      );
      for (const productId of hairCategoryDetailResult.productIds) {
        hairProductIdsFromCategories.add(productId);
      }
      ...
      hairGoodsDiagnostics.productCategoryDetailsReturned = hairCategoryDetailResult.allCategories.length;
  ...
  const hairCostFromSaleDocuments =
    (costSource === "sale_document" || costSource === "sale_document_first") && goodsMap.size > 0
      ? calculateHairGoodsCostFromGoodsMap(goodsMap, hairProductIdsFromCategories, hairProductTitleKeysFromCategories, costSource)
      : 0;
