...
async function hydrateCacheFromKv() {
  ...
  kvCacheHydrated = true;
  try {
    const snapshot = await kvRead.keycrmPipelinesSnapshot();
    if (!snapshot || !Array.isArray(snapshot.pipelines) || snapshot.pipelines.length === 0) {
      return;
    }

    const fetchedAt = snapshot.fetchedAt ?? new Date(snapshot.storedAt).toISOString();
    const expiresAt = Date.now() + CACHE_TTL_MS;

    cache = {
      data: snapshot.pipelines as KeycrmPipeline[],
      fetchedAt,
      expiresAt,
    };

    for (const pipeline of snapshot.pipelines as KeycrmPipeline[]) {
      if (pipeline?.id && Array.isArray(pipeline?.statuses) && pipeline.statuses.length) {
        rememberPipeline(pipeline, { fetchedAt, expiresAt, source: "cache" });
      }
    }
  } catch (err) {
    if (process.env.NODE_ENV !== "production") {
      console.warn("[keycrm] Failed to hydrate pipeline cache from KV", err);
    }
  }
}

async function persistPipelinesSnapshot(pipelines: KeycrmPipeline[], fetchedAt: string) {
  try {
    await kvWrite.keycrmPipelinesSnapshot({
      pipelines,
      fetchedAt,
      storedAt: Date.now(),
    });
    ...
    if (process.env.NODE_ENV !== "production") {
      console.warn("[keycrm] Failed to persist pipeline snapshot", err);
    }
  }
}
...
function normalizePipeline(raw: any): KeycrmPipeline | null {
  ...
  const statusesUnfiltered: KeycrmPipelineStatus[] = statusesRaw
    .map((status) => {
      ...
      const pipelineStatusValue = pipelineStatusId ?? rawId ?? statusId ?? primaryId;
  ...
  try {
    ...
    for (const pipeline of pipelines) {
      try {
        ...
        const hasPipelineStatusId = statuses.some((status) => status.pipelineStatusId !== null);

        if (!hasPipelineStatusId && pipeline.statuses.length) {
          const existingHasPipelineStatusId = pipeline.statuses.some((status) => status.pipelineStatusId !== null);
          if (existingHasPipelineStatusId) {
            continue;
          }
        }
    ...
    if (options.persist !== false) {
      await persistPipelinesSnapshot(pipelines, fetchedAt);
    }
