...
import type {
  ManychatRoutingError,
  ManychatRoutingSuccess,
...
export const MANYCHAT_AUTOMATION_KEY = 'manychat:last-automation';

export type ManychatAutomationSnapshot = {
  receivedAt: number;
  result: ManychatRoutingSuccess | ManychatRoutingError;
};
...
export async function persistManychatAutomation(
  result: ManychatRoutingSuccess | ManychatRoutingError,
): Promise<void> {
  const snapshot: ManychatAutomationSnapshot = {
    receivedAt: Date.now(),
    result,
  };
  ...
  if (kvClient) {
    try {
      await kvClient.set(MANYCHAT_AUTOMATION_KEY, snapshot);
      stored = true;
    } catch {
      stored = false;
    }
  ...
  if (!stored) {
    await kvWrite.setRaw(MANYCHAT_AUTOMATION_KEY, JSON.stringify(snapshot));
  }
}
...
export async function readManychatMessage(): Promise<{
  ...
  if (direct) {
    return { message: normaliseStoredMessage(direct), source: 'kv-client' };
  }
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      return { message: normaliseStoredMessage(parsed), source: 'kv-rest' };
    }
  } catch (error) {
    return {
      message: null,
...
export async function readManychatAutomation(): Promise<{
  snapshot: ManychatAutomationSnapshot | null;
  source: StoreSource | null;
  error?: string;
}> {
  const direct = await readFromKvClient<ManychatAutomationSnapshot | string>(
    MANYCHAT_AUTOMATION_KEY,
  );
  if (direct) {
    if (typeof direct === 'string') {
      try {
        const parsed = JSON.parse(direct) as ManychatAutomationSnapshot;
        return { snapshot: parsed, source: 'kv-client' };
      } catch (error) {
        return {
          snapshot: null,
  ...
  const raw = await kvRead.getRaw(MANYCHAT_AUTOMATION_KEY);
  if (!raw) {
    return { snapshot: null, source: null };
  }

  try {
    const parsed = JSON.parse(raw) as ManychatAutomationSnapshot;
    if (parsed && typeof parsed === 'object') {
      return { snapshot: parsed, source: 'kv-rest' };
    }
  } catch (error) {
    ...
    try {
      ...
      if (Array.isArray(rawList) && rawList.length) {
        const parsed = rawList
          ...
          .map((item) => normaliseStoredMessage(item));
        if (parsed.length) {
          return {
  ...
  if (rawList && rawList.length > 0) {
    ...
    for (const entry of rawList) {
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          messages.push(normaliseStoredMessage(parsed));
        }
      } catch (error) {
        return {
          messages,
