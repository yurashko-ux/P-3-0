...
export type KeycrmMoveTraceEntry = {
  step: string;
  info?: string | null;
  data?: unknown;
  timestamp: number;
...
export async function moveKeycrmCard({
  ...
  const trace: KeycrmMoveTraceEntry[] = [];
  const stack = new Error("moveKeycrmCard trace").stack ?? null;

  pushTraceEntry(trace, "moveKeycrmCard:start", null, {
    cardId,
    pipelineId,
    statusId,
    pipelineStatusId,
  ...
  const baseCandidate =
    getEnvValue("KEYCRM_API_URL", "KEYCRM_API_BASE", "KEYCRM_BASE_URL") ??
    resolveKeycrmBaseUrl();

  const tokenCandidate =
    getEnvValue("KEYCRM_API_TOKEN", "KEYCRM_TOKEN") ?? resolveKeycrmToken();
  const bearerCandidate =
    getEnvValue("KEYCRM_BEARER", "KEYCRM_API_BEARER") ?? resolveKeycrmToken();

  const authorizationVariants: string[] = [];
  const pushAuth = (value: string | null | undefined) => {
    if (!value) return;
    const trimmed = value.trim();
    if (!trimmed) return;
    if (!authorizationVariants.includes(trimmed)) {
      authorizationVariants.push(trimmed);
    }
  };

  const rawToken = tokenCandidate ?? bearerCandidate ?? resolveKeycrmToken();
  const resolvedBearer = resolveKeycrmBearer();

  if (bearerCandidate) {
    pushAuth(
      bearerCandidate.toLowerCase().startsWith("bearer ")
        ? bearerCandidate
  ...
  if (tokenCandidate) {
    pushAuth(
      tokenCandidate.toLowerCase().startsWith("bearer ")
        ? tokenCandidate
        : `Bearer ${tokenCandidate}`,
    );
    pushAuth(
      tokenCandidate.toLowerCase().startsWith("token ")
        ? tokenCandidate
        : `Token ${tokenCandidate}`,
    );
  }

  pushAuth(resolvedBearer);

  if (rawToken) {
    pushAuth(`Bearer ${rawToken}`);
    pushAuth(`Token ${rawToken}`);
  }

  const baseCandidates = buildKeycrmBaseCandidates(baseCandidate);

  pushTraceEntry(trace, "moveKeycrmCard:credentials", null, {
  ...
  if (authorizationVariants.length === 0 || baseCandidates.length === 0) {
  ...
  const finalStatusValue = statusValue ?? statusValueAlias;
  ...
  const performAttempt = async (
    ...
    const requestBodyRaw = JSON.stringify(spec.body);

    pushTraceEntry(trace, "performAttempt:start", spec.attempt, {
      base,
      url,
      method: spec.method,
    ...
    pushTraceEntry(trace, "performAttempt:response", spec.attempt, {
      status: res.status,
      ok: res.ok,
      raw: text,
    });
    ...
    if (res.ok) {
      ...
      for (let i = 0; i < maxTries; i += 1) {
        ...
        const verification = await fetchSnapshot(base, authHeader, normalisedCardId);
        ...
        pushTraceEntry(trace, "performAttempt:verification", spec.attempt, {
          attempt: i + 1,
          pipelineMatches,
          statusMatches,
          snapshot: verification,
  ...
  if (finalStatusValue !== undefined) {
    baseLegacyBody.status_id = finalStatusValue;
    baseLegacyBody.to_status_id = finalStatusValue;
  }
  ...
  // Коли є лише alias статусу, він уже врахований у finalStatusValue,
  // тому додаткові запити до інших ендпоінтів не потрібні.
  ...
  for (const authHeader of authorizationVariants) {
    for (const base of baseCandidates) {
      const baseTrimmed = base.replace(/\/+$/, "");

      for (const attemptSpec of attemptsToTry) {
      try {
          const result = await performAttempt(attemptSpec, baseTrimmed, authHeader);
        ...
        attemptHistory.push({
          ...
          requestBodyRaw: result.requestBodyRaw,
          responseRaw: result.responseRaw,
        });

        if (result.ok) {
          pushTraceEntry(trace, "moveKeycrmCard:success", attemptSpec.attempt, {
            status: result.status,
            base: baseTrimmed,
            authorization: authHeader,
          });
        ...
        if (result.status === 401) {
          pushTraceEntry(trace, "moveKeycrmCard:unauthorized", attemptSpec.attempt, {
            status: result.status,
            base: baseTrimmed,
            authorization: authHeader,
          });
        ...
        pushTraceEntry(trace, "performAttempt:error", attemptSpec.attempt, {
          base: baseTrimmed,
          authorization: authHeader,
  ...
  const fallbackHeaders =
    ...
    (lastResult?.auth ? { Authorization: lastResult.auth } : null);
  const fallbackRequestBodyRaw = lastResult?.requestBodyRaw ?? null;
  const fallbackResponseRaw = lastResult?.responseRaw ?? null;

  pushTraceEntry(trace, "moveKeycrmCard:failure", fallbackAttempt, {
    status: fallbackStatus,
    base: fallbackBase,
    url: fallbackUrl,
    authorizationTried: authorizationVariants,
  });
  ...
  return {
    ...
    requestBodyRaw: fallbackRequestBodyRaw,
    responseRaw: fallbackResponseRaw,
    trace,
    stack,
  };
}
