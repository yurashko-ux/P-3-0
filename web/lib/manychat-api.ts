...
async function loadConversationMessages(conversationId: string): Promise<ManychatLatestMessage | null> {
  ...
  > = [
    {
      path: `/instagram/conversations/${conversationId}/messages`,
    ...
    {
      path: `/instagram/conversations/${conversationId}/messages/get`,
      method: 'POST',
      body: { limit: 1, order: 'desc' },
    },
    {
    ...
    {
      path: `/conversations/${conversationId}/messages/get`,
      method: 'POST',
      body: { limit: 1, order: 'desc' },
    },
  ];
  ...
  for (const attempt of candidates) {
    try {
      const { json } = await manychatRequest<any>({
        path: attempt.path,
...
type ConversationAttempt = {
  ...
  method?: 'GET' | 'POST';
  query?: Record<string, string | number | null | undefined>;
  body?: ((limit: number) => Record<string, unknown>) | Record<string, unknown> | null;
};

const CONVERSATION_ENDPOINTS: ConversationAttempt[] = [
  { path: '/instagram/conversations', method: 'GET' },
  {
    path: '/instagram/conversations/get',
    method: 'POST',
    body: (limit) => ({ limit, order: 'desc' }),
  },
  { path: '/fb/conversations', method: 'GET' },
  {
    path: '/fb/conversations/get',
    method: 'POST',
    body: (limit) => ({ limit, order: 'desc' }),
  },
  { path: '/conversations', method: 'GET' },
  {
...
export async function fetchManychatLatest(limit = 5): Promise<{ messages: ManychatLatestMessage[]; meta: { source: 'api'; url: string } }> {
  ...
  for (const attempt of CONVERSATION_ENDPOINTS) {
    try {
      ...
      const bodyValue =
        typeof attempt.body === 'function'
          ? attempt.body(boundedLimit)
