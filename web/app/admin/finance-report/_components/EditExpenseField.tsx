...
// web/app/admin/finance-report/_components/EditExpenseField.tsx
// Універсальний компонент для редагування ручних полів витрат (захищений CRON_SECRET)

import { useState, useTransition, useEffect } from "react";
import { useRouter } from "next/navigation";

interface EditExpenseFieldProps {
  year: number;
  month: number;
  fieldKey: string; // Унікальний ключ поля (наприклад, "salary", "rent", "accounting")
  label: string; // Назва поля для відображення
  currentValue: number;
}

export function EditExpenseField({
  year,
  month,
  fieldKey,
  label,
  currentValue,
}: EditExpenseFieldProps) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const data = await res.json();
        console.log(`[EditExpenseField] Saved ${fieldKey}:`, data);
        ...
        console.error(`[EditExpenseField] Save error:`, err);
