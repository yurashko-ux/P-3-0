...
/** Діапазон висот стовпців (понад baseline) у видимій вибірці — для помітної різниці 156 vs 160. */
function computeActiveBaseHeightRange(values: number[]): { min: number; max: number } {
...
/** Висота стовпця у % від зони графіка: min → 6%, max → 100%. */
function getActiveBaseBarHeightPct(value: number, min: number, max: number): number {
  if (!Number.isFinite(value)) return 6;
  if (max <= min) return 100;
  const normalized = (value - min) / (max - min);
  return 6 + normalized * 94;
}
...
function ActiveBaseMonthlyChart({
  ...
  const barHeightValues = points.map((p) => getActiveBaseBarHeightValue(p));
  const heightRange = computeActiveBaseHeightRange(barHeightValues);

  return (
    <ActiveBaseChartShell
      title="Активна база: з початку року"
      subtitle={`Останній snapshot у кожному місяці. Висота — клієнти понад ${ACTIVE_BASE_CHART_BASELINE}, масштаб у межах періоду. Кольорова зміна — від ${ACTIVE_BASE_CHART_BASELINE} клієнтів.`}
      loading={loading}
      error={error}
    >
      {points.length === 0 ? (
        ...
        <div className="h-60 flex gap-2 border-b border-base-300 px-1 pt-3">
          {points.map((p) => {
            ...
            const heightPct = getActiveBaseBarHeightPct(barHeightValue, heightRange.min, heightRange.max);
...
function ActiveBaseDailyChart({
  ...
  const visibleBarHeightValues = visiblePoints.map((p) => getActiveBaseBarHeightValue(p));
  const heightRange = computeActiveBaseHeightRange(visibleBarHeightValues);
  ...
  return (
    <ActiveBaseChartShell
      ...
      subtitle={`${subtitle}. Висота — клієнти понад ${ACTIVE_BASE_CHART_BASELINE}, масштаб у межах видимого періоду. Кольорова зміна — від ${ACTIVE_BASE_CHART_BASELINE} клієнтів. Колесо — масштаб, Shift + колесо — прокрутка по часу.`}
      loading={loading}
      error={error}
    >
      {sortedPoints.length === 0 ? (
        ...
        <>
          ...
          <div
            className="h-60 flex gap-1 border-b border-base-300 px-1 pt-3 overflow-hidden cursor-ew-resize"
            onWheel={handleWheel}
          >
            {visiblePoints.map((p) => {
...
function DirectStatsPageContent() {
  ...
  /** Колонка «З початку місяця»: основний рядок — оборот мінус знижка (тис.); дрібно — сума знижки. */
  const renderMtdAfterDiscountCell = (grossUAH: number, discountUAH: number, loading: boolean) => {
    if (loading) return <span className="tabular-nums">…</span>;
    const net = Math.max(0, grossUAH - discountUAH);
    return (
      <div className="flex flex-col items-center justify-center gap-0 leading-tight">
        <span className="tabular-nums">{formatFutureThousands(net)}</span>
        <span className="text-[6px] opacity-80 tabular-nums mt-0.5 block">
          {formatDiscountSubline(discountUAH) || '0 грн'}
        </span>
      </div>
    );
  };
            ...
            <div className="card-body p-4 w-full min-w-0">
              ...
              <div className="overflow-x-auto w-full">
                      ...
                      <table className="table table-xs border-separate border-spacing-0 text-[7px] w-full table-fixed">
                        ...
                        <tbody>
                          <tr>
                            <td data-cell="B28" data-block={monthStatsBlockId} className="font-medium align-middle">
                              Майбутні записи
                            </td>
                            <td
                              data-cell="C28"
                            ...
                            <td
                              data-cell="E28"
                              data-block={monthStatsBlockId}
                              className="text-center tabular-nums font-medium align-middle"
                          ...
                          {futureExcelRows.map(({ name, row: mr }, i) => {
                            const row = 29 + i;
                            const cGross = mr?.turnoverMonthToDateUAH ?? 0;
                            const cDisc = mr?.discountMonthToDateUAH ?? 0;
                            const cNet = Math.max(0, cGross - cDisc);
                            const d = mr?.monthToEndSum;
                            const e = cNet + (mr?.monthToEndSum ?? 0);
                            const f = mr?.nextMonthSum;
                            const g = mr?.plus2MonthSum;
                            return (
                              <tr key={name}>
                                <td data-cell={`B${row}`} data-block={monthStatsBlockId} className="font-medium align-middle">
                                  {name}
                                </td>
                                <td
                                  data-cell={`C${row}`}
                                  ...
                                  title={
                                    mr
                                      ? `Після знижки: ${formatUAHExact(cNet)} · брутто ${formatUAHExact(cGross)} · знижка ${formatUAHExact(cDisc)}`
                                ...
                                >
                                  {mastersStats.loading && !hasFutureStatsData
                                    ...
                                    : !mr
                                      ? "—"
                                      : renderMtdAfterDiscountCell(cGross, cDisc, false)}
                                </td>
                                <td
                                  data-cell={`D${row}`}
                                  data-block={monthStatsBlockId}
                          ...
                          {showUnassignedFutureRow && unassignedStatsRow ? (
                            <tr key="unassigned-future">
                              <td data-cell="B33" data-block={monthStatsBlockId} className="font-medium text-base-content/80 align-middle">
                                Без майстра
                              </td>
                              <td
                                data-cell="C33"
                              ...
                              >
                                {mastersStats.loading && !hasFutureStatsData
                                  ...
                                  : renderMtdAfterDiscountCell(
                                      ...
                                      unassignedStatsRow.discountMonthToDateUAH ?? 0,
                                      false,
                                    )}
                              </td>
                              <td
