...
export function StatusFilterDropdown({
  ...
  // Показуємо всі створені статуси (без фільтра usedIds)
  ...
  const [pending, setPending] = useState<string[]>(filters.statusIds ?? []);

  useEffect(() => {
    setPending(filters.statusIds ?? []);
  }, [filters.statusIds]);
  ...
  }, [isOpen, filters.statusIds]);

  const hasActive = (filters.statusIds ?? []).length > 0;
  ...
  const handleApply = () => {
    onFiltersChange({ ...filters, statusIds: pending });
    setIsOpen(false);
  };
  ...
  const panelContent = (
    <div className="p-2">
      ...
      <div className="space-y-1">
        {options.length === 0 ? (
          <div className="px-2 py-1.5 text-xs text-gray-500">Немає створених статусів</div>
        ) : (
          options.map((opt) => {
