...
export default function NewCampaignPage() {
  ...
  const canSubmit = useMemo(() => {
    ...
    const expOk = !expToPipelineId || !!expToStatusId;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // ВАЖЛИВО: спочатку create, сувора перевірка JSON.ok === true
      const targets = ['/api/campaigns/create', '/api/campaigns'];
      ...
      for (const u of targets) {
        ...
        if (res.ok && j && j.ok === true) { ok = true; break; }
        lastErr = (j && j.error) || `${res.status} ${res.statusText}`;
      }

      if (!ok) throw new Error(lastErr || 'save failed');

      alert('Кампанію збережено');
      // PRG + вимушене оновлення списку
