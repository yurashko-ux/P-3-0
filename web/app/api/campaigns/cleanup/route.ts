...
import { campaignKeys } from "@/lib/kv";
...
async function gatherCampaignIds(): Promise<{ ids: string[]; sources: Record<string, number> }> {
  ...
  const cmpIndex = unwrapDeep(await kv.get(campaignKeys.CMP_INDEX_KEY));
  register(campaignKeys.CMP_INDEX_KEY, cmpIndex);

  const cmpList = await kv
    .lrange<string | number>(campaignKeys.CMP_INDEX_KEY, 0, -1)
    .catch(() => []);
  register(`${campaignKeys.CMP_INDEX_KEY}:list`, cmpList);

  const campaignIndex = unwrapDeep(await kv.get(campaignKeys.INDEX_KEY));
  register(campaignKeys.INDEX_KEY, campaignIndex);

  const legacyIndex = unwrapDeep(await kv.get(campaignKeys.LEGACY_INDEX_KEY));
  register(campaignKeys.LEGACY_INDEX_KEY, legacyIndex);

  for (const key of LEGACY_COLLECTION_KEYS) {
    const value = unwrapDeep(await kv.get(key));
    register(key, value);
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    const { ids, sources } = await gatherCampaignIds();
    ...
    const indexDeletion = await deleteKeys([...itemKeys, ...INDEX_KEYS]);
