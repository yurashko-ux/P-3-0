...
import { campaignKeys, getKvConfigStatus, kvRead } from "@/lib/kv";
...
async function gatherCampaignIds(kvReadable: boolean): Promise<GatherResult> {
  ...
  const safeList = async (key: string): Promise<(string | number)[]> => {
    try {
      const value = await kvRead.lrange(key, 0, -1);
  ...
  const cmpList = await safeList(campaignKeys.CMP_INDEX_LIST_KEY);
  register(campaignKeys.CMP_INDEX_LIST_KEY, cmpList);
