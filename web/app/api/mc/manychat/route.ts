...
export async function GET() {
  ...
  const serialiseMessage = (message: LatestMessage): LatestMessage => {
    const rawReceived = message.receivedAt as unknown;
    const receivedAt =
      typeof rawReceived === 'number' && Number.isFinite(rawReceived)
        ? rawReceived
        : typeof rawReceived === 'string'
          ? (() => {
              const trimmed = rawReceived.trim();
    ...
    return {
      ...
      id: normalisedId,
      receivedAt,
      source: message.source ?? 'manychat',
      title:
        typeof message.title === 'string' && message.title.trim().length
          ? message.title
          : 'ManyChat',
      handle: message.handle ?? null,
      fullName: message.fullName ?? null,
  ...
  const safeFeed = feed.map(serialiseMessage);
  const safeLatest = latest ? serialiseMessage(latest) : null;
