...
function fromTraceToStoredMessage(trace: WebhookTrace): ManychatStoredMessage {
  const receivedAtCandidate =
    typeof trace.receivedAt === 'number' && Number.isFinite(trace.receivedAt)
      ? trace.receivedAt
      : typeof trace.receivedAt === 'string' && Number.isFinite(Number(trace.receivedAt))
        ? Number(trace.receivedAt)
        : Date.now();

  return {
    id: receivedAtCandidate,
    receivedAt: receivedAtCandidate,
    source: 'trace:webhook',
    title: 'ManyChat Webhook (trace)',
    handle: trace.handle ?? null,
    fullName: trace.fullName ?? null,
    text: trace.messagePreview ?? '',
    raw: { trace },
  };
}
...
export async function GET() {
  ...
  if (feed.length === 0 && trace) {
    ...
    if (fallbackText || fallbackHandle || fallbackFullName) {
      const storedFallback = fromTraceToStoredMessage(trace);
      let kvMessageStored = false;
      let kvFeedStored = false;
      let kvStoreError: string | null = null;
      let kvFeedError: string | null = null;

      try {
        await persistManychatSnapshot(storedFallback, trace);
        kvMessageStored = true;
      } catch (error) {
        kvStoreError = error instanceof Error ? error.message : String(error);
      }

      try {
        kvFeedStored = await ensureManychatFeedSnapshot([storedFallback]);
      } catch (error) {
        kvFeedError = error instanceof Error ? error.message : String(error);
      }

      const fallbackMessage: LatestMessage = {
        id: storedFallback.id,
        receivedAt: storedFallback.receivedAt,
        source: kvMessageStored || kvFeedStored ? 'kv' : 'trace:webhook',
        title: storedFallback.title,
        handle: fallbackHandle,
        fullName: fallbackFullName,
        text: fallbackText,
        raw: storedFallback.raw,
      };

      feed = [fallbackMessage];
      latest = fallbackMessage;
      source = (kvMessageStored || kvFeedStored) ? 'kv' : source ?? 'trace';
      ...
      if (kvMessageStored) {
        diagnostics.kv = {
          ...
          key: MANYCHAT_MESSAGE_KEY,
          source: 'kv',
          message: 'Fallback вебхука збережено у Vercel KV.',
        };
      } else if (kvStoreError && !diagnostics.kv) {
        diagnostics.kv = {
          ok: false,
          key: MANYCHAT_MESSAGE_KEY,
          source: 'error',
          message: `Не вдалося зберегти fallback у KV: ${kvStoreError}`,
        };
      }
      ...
      if (kvFeedStored) {
        diagnostics.kvFeed = {
          ...
          source: 'kv',
          count: 1,
          message: 'Журнал відновлено за даними останнього вебхука.',
        };
      } else if (kvFeedError && (!diagnostics.kvFeed || diagnostics.kvFeed.source === 'miss')) {
        diagnostics.kvFeed = {
          ok: false,
          key: MANYCHAT_FEED_KEY,
          source: 'error',
          message: `Не вдалося оновити журнал у KV: ${kvFeedError}`,
        };
      }
      ...
      diagnostics.traceFallback = {
        used: true,
        reason:
          kvMessageStored || kvFeedStored
            ? 'Вебхук збережено та показано з кешу трасування, оскільки основний журнал був порожній.'
            : 'Відображаємо останній вебхук із трасування, оскільки повідомлення не знайдено у KV або ManyChat API.',
      };
    }
  }
