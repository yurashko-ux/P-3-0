...
function toKey(message: LatestMessage): string {
  const idPart =
    typeof message.id === 'string' && message.id.trim().length
      ...
      : typeof message.id === 'number' && Number.isFinite(message.id)
        ? `#${message.id}`
        : '';
  const handlePart = message.handle ? message.handle.toLowerCase() : '';
  const tsPart =
    typeof message.receivedAt === 'number' && Number.isFinite(message.receivedAt)
      ? String(message.receivedAt)
      : '';

  return [idPart, handlePart, tsPart].filter(Boolean).join('|') || tsPart || idPart || handlePart;
}

function mergeFeeds(primary: LatestMessage[], secondary: LatestMessage[]): LatestMessage[] {
  const merged = new Map<string, LatestMessage>();
  ...
  result.sort((a, b) => {
    const aTs =
      typeof a.receivedAt === 'number' && Number.isFinite(a.receivedAt)
        ? a.receivedAt
        : typeof a.receivedAt === 'string'
          ? Number(a.receivedAt)
          : 0;
    const bTs =
      typeof b.receivedAt === 'number' && Number.isFinite(b.receivedAt)
        ? b.receivedAt
...
export async function GET() {
  ...
  let source: string | null = lastMessage ? 'memory' : null;
  ...
  if (apiKeyAvailable) {
    try {
      ...
      if (messages.length > 0) {
        ...
        feed = mergeFeeds(apiFeed, feed);
        latest = feed[0] ?? latest ?? null;
        source = source ? `${source}+api` : 'api';
        diagnostics.api = {
          ok: true,
          url: meta.url,
          note: beforeCount > 0 ? 'merged' : 'fetched',
          merged: feed.length,
        };
      } else {
