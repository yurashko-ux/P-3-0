...
function serialiseMessage(message: LatestMessage): LatestMessage {
  const receivedAt =
    typeof message.receivedAt === 'number' && Number.isFinite(message.receivedAt)
      ? message.receivedAt
      : Date.now();

  let id: number | string = receivedAt;
  if (typeof message.id === 'number' && Number.isFinite(message.id)) {
    id = message.id;
  } else if (typeof message.id === 'string' && message.id.trim().length) {
    id = message.id.trim();
  }
  ...
  return {
    ...
    title: message.title && message.title.trim().length ? message.title : 'ManyChat',
...
export async function GET() {
  ...
  const safeFeed = feed.map((item) => serialiseMessage(item));
