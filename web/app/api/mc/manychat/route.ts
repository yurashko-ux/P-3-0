...
export async function GET() {
  ...
  const enhanceForResponse = (message: LatestMessage): LatestMessage => {
    const hydratedText = message.text && message.text.trim().length
      ? message.text
      : trace?.messagePreview && trace.messagePreview.trim().length
        ? trace.messagePreview
        : '';

    const hydratedHandle = message.handle ?? trace?.handle ?? null;
    const hydratedFullName = message.fullName ?? trace?.fullName ?? null;

    return {
      ...message,
      text: hydratedText,
      handle: hydratedHandle,
      fullName: hydratedFullName,
    };
  };

  const responseFeed = feed.map(enhanceForResponse);
  const responseLatest = latest ? enhanceForResponse(latest) : null;

  return NextResponse.json({
    ok: true,
    latest: responseLatest,
    feed: responseFeed,
    messages: responseFeed,
    source,
    trace,
    diagnostics,
  });
