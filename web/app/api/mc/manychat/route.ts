...
export async function GET() {
  ...
  if (feed.length > 0) {
    if (!diagnostics.kvFeed) {
      diagnostics.kvFeed = {
        ok: true,
        key: MANYCHAT_FEED_KEY,
        source: 'memory',
        count: feed.length,
        message: 'Журнал побудовано з останнього вебхука',
      };
    } else if (
      diagnostics.kvFeed.source === 'miss' ||
      diagnostics.kvFeed.source === 'error'
    ) {
      diagnostics.kvFeed = {
        ok: true,
        key: MANYCHAT_FEED_KEY,
        source: 'memory',
        count: feed.length,
        message: diagnostics.kvFeed.message
          ? `${diagnostics.kvFeed.message}; журнал побудовано з останнього вебхука`
          : 'Журнал побудовано з останнього вебхука',
      };
    }
  }
