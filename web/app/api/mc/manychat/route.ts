...
function normaliseForResponse(
  message: LatestMessage,
  trace: WebhookTrace | null,
  fallbackSource: string | null,
): LatestMessage {
  const trimmedTitle = typeof message.title === 'string' ? message.title.trim() : '';

  const ensuredId = (() => {
    if (typeof message.id === 'number' && Number.isFinite(message.id)) {
      return message.id;
    }
    if (typeof message.id === 'string' && message.id.trim().length) {
      return message.id.trim();
    }
    if (typeof message.receivedAt === 'number' && Number.isFinite(message.receivedAt)) {
      return message.receivedAt;
    }
    return Date.now();
  })();

  const ensuredTimestamp = (() => {
    const rawReceivedAt = message.receivedAt as unknown;
    if (typeof rawReceivedAt === 'number' && Number.isFinite(rawReceivedAt)) {
      return rawReceivedAt;
    }
    if (typeof rawReceivedAt === 'string') {
      const numeric = Number(rawReceivedAt.trim());
  ...
  const ensuredText = (() => {
    ...
    if (typeof trace?.messagePreview === 'string' && trace.messagePreview.trim().length) {
      return trace.messagePreview;
    }
    if (typeof message.raw === 'string' && message.raw.trim().length) {
      return message.raw;
    }
    return '';
  })();
  ...
  const ensuredHandle = (() => {
    if (typeof message.handle === 'string' && message.handle.trim().length) {
      return message.handle.trim();
    }
    if (typeof trace?.handle === 'string' && trace.handle.trim().length) {
      return trace.handle.trim();
    }
    return null;
  })();

  const ensuredFullName = (() => {
    if (typeof message.fullName === 'string' && message.fullName.trim().length) {
      return message.fullName.trim();
    }
    if (typeof trace?.fullName === 'string' && trace.fullName.trim().length) {
      return trace.fullName.trim();
    }
    return null;
  })();

  const ensuredSource = (() => {
    if (typeof message.source === 'string' && message.source.trim().length) {
      return message.source.trim();
    }
    if (fallbackSource) {
  ...
  return {
    ...message,
    id: ensuredId,
    receivedAt: ensuredTimestamp,
    text: ensuredText,
    handle: ensuredHandle,
    fullName: ensuredFullName,
    source: ensuredSource,
    title: trimmedTitle.length ? trimmedTitle : 'ManyChat',
  };
}
...
export async function GET() {
  ...
  const normalisedFeed = feed.map((entry) => normaliseForResponse(entry, trace, responseSource));
  const normalisedLatest = latest ? normaliseForResponse(latest, trace, responseSource) : null;

  return NextResponse.json({
    ok: true,
    latest: normalisedLatest,
    feed: normalisedFeed,
