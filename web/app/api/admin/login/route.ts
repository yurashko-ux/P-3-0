// web/app/api/admin/login/route.ts
import { NextResponse } from 'next/server';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

function seeOther(url: URL) {
  return NextResponse.redirect(url, { status: 303 }); // POST → GET
}

export async function GET(req: Request) {
  // Якщо відкрили API напряму — ведемо на форму
  return seeOther(new URL('/admin/login', req.url));
}

export async function POST(req: Request) {
  const adminPass = (process.env.ADMIN_PASS || process.env.ADMIN_PASSWORD || '').trim();
  if (!adminPass) return seeOther(new URL('/admin/login?err=env', req.url));

  const form = await req.formData().catch(() => null);
  const input = String(form?.get('password') || '').trim();

  if (input && input === adminPass) {
    const res = seeOther(new URL('/admin', req.url));
    res.cookies.set('admin_ok', '1', {
      httpOnly: true,
      sameSite: 'lax',
      secure: true, // на Vercel ок
      path: '/',
      maxAge: 60 * 60 * 24 * 30,
    });
    return res;
  }

  return seeOther(new URL('/admin/login?err=1', req.url));
}
