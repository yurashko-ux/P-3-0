...
export async function POST(req: NextRequest) {
  ...
  const automation = await routeManychatMessage({
    ...
    performMove: async ({
      ...
      const moveUrl = new URL("/api/keycrm/card/move", req.nextUrl.origin);

      const body: Record<string, unknown> = { card_id: String(cardId) };

      if (pipelineId != null) {
        body.to_pipeline_id = pipelineId;
      }

      if (statusId != null) {
        body.to_status_id = statusId;
      }

      if (pipelineStatusId != null) {
        body.pipeline_status_id = pipelineStatusId;
      }

      if (statusAliases && statusAliases.length) {
        body.status_aliases = statusAliases;
      }
      ...
      try {
        ...
        if (!res.ok || !json?.ok) {
          return {
            ...
            sent: (json && typeof json === "object" ? (json as any).sent : null) ?? null,
            attempts: Array.isArray((json as any)?.attempts) ? ((json as any)?.attempts as any[]) : undefined,
            requestUrl: typeof (json as any)?.requestUrl === "string" ? ((json as any)?.requestUrl as string) : null,
            requestMethod:
              typeof (json as any)?.requestMethod === "string" ? ((json as any)?.requestMethod as string) : null,
          };
        }
