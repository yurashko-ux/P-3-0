...
async function sendViaManyChat(
  ...
  try {
    ...
    console.log(`[reminders] ===== METHOD 1: getSubscribers with ig_username filter =====`);
    try {
      ...
      for (let page = 1; page <= maxPages; page++) {
        ...
        console.log(`[reminders] Fetching page ${page} from getSubscribers...`);
        ...
        if (subscribersResponse.ok) {
          ...
          console.log(`[reminders] Page ${page}: received ${subscribers.length} subscribers`);
          
          // Логуємо перші кілька subscribers для діагностики
          if (page === 1 && subscribers.length > 0) {
            const firstSub = subscribers[0];
            console.log(`[reminders] Sample subscriber structure:`, JSON.stringify({
              id: firstSub.id,
              name: firstSub.name,
          ...
          const foundSubscriber = subscribers.find((sub: any) => {
            ...
            if (subIgUsername) {
              console.log(`[reminders] Checking subscriber ${sub.id}: ig_username="${subIgUsername}" vs search="${searchIgUsername}" -> ${match ? 'MATCH!' : 'no match'}`);
            }
          ...
          if (foundSubscriber) {
            ...
            console.log(`[reminders] ✅ Found subscriber via getSubscribers on page ${page}: ${subscriberId}`, JSON.stringify(foundSubscriber, null, 2));
            break;
          }
          
          // Якщо на цій сторінці менше ніж pageSize, це остання сторінка
          if (subscribers.length < pageSize) {
            console.log(`[reminders] Last page reached (${subscribers.length} < ${pageSize})`);
            break;
          }
        } else {
          const errorText = await subscribersResponse.text();
          console.warn(`[reminders] getSubscribers failed on page ${page}: ${subscribersResponse.status} ${errorText.substring(0, 200)}`);
          break;
        }
      }
      
      if (!subscriberId) {
        console.warn(`[reminders] ❌ Subscriber not found via getSubscribers after checking up to ${maxPages} pages`);
      }
    } catch (err) {
      console.error(`[reminders] getSubscribers error:`, err);
    }
