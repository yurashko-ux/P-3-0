...
function getAltegioBalanceDisplay(item: OperationItem): {
  ...
  if (item.altegioBalanceFromAnchor != null) {
    ...
    return {
      ...
      title:
        "Оціночний баланс грошового рахунку в Altegio: сума станом на кінець дня Europe/Kyiv дати відліку (адмінка Altegio → Банк ↔ Altegio), плюс операції Monobank після цього дня до цієї операції включно. Не враховує рухи лише в Altegio без відображення в monobank.",
      color: "#6d28d9",
    };
  }
...
function BankFooterAccountStrip({ row }: { row: BankFooterStripAccount }) {
  ...
  const abTitle = row.altegioIsEstimate
    ? "Altegio: оцінка (точка відліку + Monobank після кінця дня Europe/Kyiv)"
