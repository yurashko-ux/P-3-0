...
3. `lastActivityKeys` не порожній
...
- Endpoint `sync-consultation-for-client` може робити `lastActivityKeysRepair` — додає `consultationAttended` або `paidServiceRecordCreatedAt`, якщо запис створений сьогодні, але ключа немає в `lastActivityKeys`
