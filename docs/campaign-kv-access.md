...
> Якщо потрібна швидка відповідь «що робити просто зараз», скористайтесь [короткою пам'яткою](./kv-checklist.md). У ній зібрані готові команди з цими ж токенами і стислий порядок дій. Нижче — розгорнуте пояснення та повний довідник.
...
### Типові причини збоїв доступу до KV

1. **Невідповідні креденшіали.** Найчастіше REST-запит повертає `401/403`, коли токен прив'язаний до іншого середовища, втратив чинність або має лише `read`-права і блокує запис (`SET`/`LPUSH`). У такому випадку потрібно перевипустити токен у Vercel або Upstash чи видати додаткові привілеї.
2. **Порожні змінні середовища.** Якщо `KV_REST_API_URL` / `KV_REST_API_TOKEN` (або `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`) не задані в конкретному середовищі деплою чи локальному `.env`, клієнт просто не знає, куди звертатись, і миттєво повертає fallback зі словом про відсутність кампаній.
3. **Відсутній прямий биндинг.** Коли застосунок розгортається без ресурсу `@vercel/kv`, то після вичерпання REST-спроб лишається лише офлайн-дамп (якщо його не завантажено — кампанії також не підтягуються). Перевірте налаштування `vercel.json` або панелі Vercel, щоб ресурс `kv` був приєднаний до проєкту.
4. **Блокування мережевого виходу.** На локальних машинах або CI може бути обмежено доступ до інтернету, через що REST-запити закінчуються тайм-аутами. У такому випадку використовуйте офлайн-снепшот (`KV_CAMPAIGNS_SNAPSHOT`) або проксування мережі.

Якщо одразу після перевірки `curl .../ping` бачите `200 OK`, але будь-які читання (`LRANGE`, `GET`) повертають `403 Forbidden`, значить токен справді чинний, але без права на відповідну операцію (наприклад, лише читання). Якщо ж запит закінчується на `400 Bad Request`, майже завжди проблема в синтаксисі команди або URL-енкодингу (докладні приклади наведені нижче).
...
### CLI-команда для пошуку без HTTP-запиту

Якщо потрібно виконати пошук прямо з репозиторію, скористайтеся готовим CLI-скриптом. Він підтягує кампанії через `kvRead.listCampaigns`, нормалізує значення правил та повертає JSON зі збігами:

```bash
# точний збіг за V1
npm run find:campaign -- --value 1 --slot v1 --match equals

# аналогічно — перевірка правила V2
npm run find:campaign -- --value 2 --slot v2 --match equals
```

Опції ті самі, що й у REST-ендпойнта: `--value` (обовʼязково), `--slot` (`v1` або `v2`, можна вказати кілька разів — наприклад, щоб перевірити обидва слоти однією командою), `--match` (`equals` або `contains`, за замовчуванням `contains`) та `--active` для відсікання неактивних кампаній. У відповіді повертається список знайдених кампаній разом із усіма нормалізованими значеннями правил по слотах.
