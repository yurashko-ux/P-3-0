...
4. Відповідь `400 Bad Request` з текстом `ERR wrong number of arguments for 'keys' command` означає, що сервер отримав некоректну кількість аргументів (наприклад, коли використати запит `/keys?limit=1`). Приберіть параметри, залиште лише шлях `/keys/%2A` або скористайтеся командою `SCAN` (див. нижче) — у такому разі токен буде перевірено коректно.
...
>   "https://hot-louse-21041.upstash.io/keys/%2A"
> ```
>
