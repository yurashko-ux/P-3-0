...
Опції ті самі, що й у REST-ендпойнта: `--value` (обовʼязково), `--slot` (`v1` або `v2`, можна кілька разів), `--match` (`equals` або `contains`, за замовчуванням `contains`) та `--active` для відсікання неактивних кампаній. Результат містить список знайдених кампаній разом із усіма нормалізованими значеннями правил по слотах.
