...
| **3. Позначка в БД** | Поля **`consultationDeletedInAltegio`** та **`paidServiceDeletedInAltegio`** = true. Вебхуки та sync/cron **не перезаписують** консультацію/запис, якщо відповідний прапорець встановлено. Стан «видалено» зберігається до появи **нового** запису в Altegio. |
| **4. UI** | У колонках «Консультація» та «Запис» для клієнтів з відповідним `*DeletedInAltegio = true` показується «Видалено в Altegio» замість порожньої комірки. |
...
### 4.6. Позначки «Видалено в Altegio» (consultationDeletedInAltegio, paidServiceDeletedInAltegio)

- У БД зберігаються поля **`consultationDeletedInAltegio`** та **`paidServiceDeletedInAltegio`** (boolean). Коли консультацію або платний запис очищають через відсутність візиту в Altegio (404), відповідне поле встановлюється в **true**.
- **Вебхуки Altegio** та **sync/cron**, які знову встановлюють дату консультації або майстра, перевіряють цей прапорець: якщо він **true**, таке оновлення **пропускається** (консультація не перезаписується). Таким чином стан «видалено» зберігається, поки в Altegio не з’явиться **новий** запис.
- Коли з будь-якого джерела знову встановлюють **не-null** дату консультації або платного запису, виставляють відповідний прапорець у **false**.
- У таблиці Direct у колонках «Консультація» та «Запис» для клієнтів без дати, але з відповідним `*DeletedInAltegio = true`, показується **«Видалено в Altegio»** (сірий, курсив).
